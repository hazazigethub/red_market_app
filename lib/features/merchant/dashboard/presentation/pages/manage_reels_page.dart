  ...
  Future<void> _handleUpload(String title, String desc, XFile videoFile,
    ...
    if (fileSize > 100 * 1024 * 1024) {
      messenger.showSnackBar(const SnackBar(
          content: Text("حجم الملف الخام كبير جداً قبل الضغط!")));
      return;
    }
    ...
    try {
      ...
      if (info.duration != null) {
        if (info.duration! > 20500) {
          if (mounted) {
            setState(() => _isUploading = false);
            _showErrorDialog("المقطع طويل جداً ⏱️",
                "سياسة التطبيق تسمح بـ 20 ثانية فقط. مدة مقطعك الحالي ${(info.duration! / 1000).toStringAsFixed(1)} ثانية.");
          }
          return;
        }
        ...
        if (info.duration! < 2000) {
          if (mounted) {
            setState(() => _isUploading = false);
            _showErrorDialog(
                "الفيديو قصير جداً", "يجب أن يكون المقطع ثانيتين على الأقل.");
          }
          return;
        }
      }
      ...
      if (mediaInfo == null || mediaInfo.file == null) {
        throw "فشل في معالجة وضغط الفيديو";
      }
